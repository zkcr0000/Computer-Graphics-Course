        ...
        for ( int j = y_min; j < y_max; j++)
        {
            if (insideTriangle(i+0.5,j+0.5,t.v))
            {
                ...
                if (z_interpolated < depth_buf[get_index(i, j)])
                {
                    ...
                    // also interpolate the view position, 
                    // view_pos[0,1,2] are the triangle vertex position after Model/View transformation
                    // So the estimated position of current pixel in the 3D space is calculated
                    auto interpolated_shadingcoords = interpolate(alpha, beta, gamma, view_pos[0], view_pos[1], view_pos[2], 1.0);
                    // set the things needed for fragment shader
                    fragment_shader_payload payload( interpolated_color, interpolated_normal.normalized(), interpolated_texcoords, texture ? &*texture : nullptr);
                    // set the current pixel estimated position in 3D space
