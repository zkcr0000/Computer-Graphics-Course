                                ...
                                const std::array<int, 3>& dirIsNeg) const
{
    ...
    // pMin and pMax are two points to specify the bounding box
    // pMin has x,y,z value smaller than pMax
    // A plane can be specified by a point and normal vector
    // pMin and pMax each specifies three surfaces of bounding box by normal vectors along x,y,z axis

    // ray with direction d and origin o (o+td), intersects with a plane specified by point p and normal vector along x,y,z axis
    // result would be t = (p_x-o_x)/d_x
